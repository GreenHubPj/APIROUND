document.addEventListener('DOMContentLoaded', function() {
    ...
    function searchRecipes(searchTerm) {
        recipeCards.forEach(card => {
            const recipeNameEl = card.querySelector('.recipe-name');
            const recipeDescEl = card.querySelector('.recipe-description');
            const recipeName = recipeNameEl ? recipeNameEl.textContent.toLowerCase() : '';
            const recipeDescription = recipeDescEl ? recipeDescEl.textContent.toLowerCase() : '';
            const cardCategory = card.getAttribute('data-category');
    ...
    function highlightSearchTerm(card, searchTerm) {
        ...
        if (recipeName) {
            ...
            if (recipeName.textContent.toLowerCase().includes(searchTerm)) {
                ...
                recipeName.innerHTML = recipeName.textContent.replace(
                    regex,
        ...
        if (recipeDescription) {
            ...
            if (recipeDescription.textContent.toLowerCase().includes(searchTerm)) {
                const regex = new RegExp(`(${searchTerm})`, 'gi');
                recipeDescription.innerHTML = recipeDescription.textContent.replace(
                    regex,
                    '<mark style="background-color: #ffeb3b; padding: 2px 4px; border-radius: 3px;">$1</mark>'
                );
            }
